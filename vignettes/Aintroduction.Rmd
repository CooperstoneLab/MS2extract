...
MS2extract computes the theoretical precursor *m*/*z*. This information extracts
product ion scans that match the theoretical ionized mass
with a given ppm tolerance.
...
a MS/MS library format (.msp/.mgf) to be used as a reference library for
...
consists of importing scan data in a list (S3 object) rather than into a S4
object, facilitating the downstream tidy analysis of this object.

> This function executes a back-end calculation of theoretical ionized *m*/*z*
...
the compound elutes, in order to only keep this retention time window.
...
Therefore, it is more accurate to interpret this chromatogram as 
...
have to meet to be kept in the data, while signals that do not meet the defined
threshold are removed.  This function can also normalize the spectra ion 
intensity to a percentage, based on the base peak. This is a filtering step 
that is based on the percentage of the base peak (most intense ion).

The three required arguments are:

* **spec:** a data frame containing the MS/MS spectra.
* **normalize:** a boolean indicating if the MS/MS spectra are normalized by the base peak before proceeding to filter out low intensity signals (normalize  = TRUE), if normalize = FALSE the user has to provide the minimum ion count.
...
We can now see the range of m/z values and the maximum value of 576.1221 m/z.
...
If we take a look at the previous MS/MS plot, there is less background noise
...
* **spec_metadata:** a data frame containing the values to be included in the resulting .msp file
