---
title: "Introduction to MS2extract package"
...
output: 
  rmarkdown::html_vignette:
    ...
    fig_caption: true      
vignette: >
  %\VignetteIndexEntry{Introduction to MS2extract package}
...
formula, the m/z is calculated extract the MS/MS scans matching with the 
theoretical m/z. Then, low intensity signals, or background noise, is removed
out of the spectra. Finally, user can export the extracted MS/MS spectra to
a msp file format to be used as reference library for further compound
identification and annotation.


```{r Fig 1, echo=FALSE, fig.cap= "Figure 1. Overview of general data processing pipeline to extract MS/MS exptra using the MS2extract package", out.width= "80%", out.height="80%", message=FALSE, warning=FALSE, paged.print=FALSE, fig.align='center'}
...
that user may need to provide in order to properly use this package.
...
object, facilitating the downstream tidy analysis of this objects.
...
File consist in the file name of your mzXML file that contains MS/MS data
of your authentic standards or reference material. Here, we provide an example
...
to provide the Region Of Interest where the compound elutes in order to only
...
scans and select the scans with the highest intensities.
...
signal intensity threshold that ions has to meet in order to be kept. Thus, ion
