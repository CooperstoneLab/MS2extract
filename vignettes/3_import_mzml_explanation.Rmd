...
Although providing a retention time window, it is highly suggested to
have more control over the regions of the run that we look for the MS/MS
scans. If you do not provide for a specific rt ROI, this package will
look for the most intense scan, even if that do not represent the
desired scans from the provided metabolite.

This is a case example, that you can trim the retention time for the
specific elution time of the metabolite.
...
In the following example, the scan in the first does not meet these
requierements, and it is discard, while the second and third scan falls
within these requierements, and are keept for the next steps.
